...
namespace V1API_ConsoleApp
{
    ...
    class Program
    {
        ...
        static string getXML(string xmlType,string i, string[] user)
        {
            ...
            {
                returnXML = @"<Asset href='/api-examples/rest-1.v1/New/Member'><Attribute name='Name' act='set'>" + user[0] +
                                "</Attribute><Attribute name='Nickname' act='set'>" + user[1] +
                                "</Attribute><Attribute name='Username' act='set'>" + @"Healthfirst\" + user[2] +
